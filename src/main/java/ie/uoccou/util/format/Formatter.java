...
public class Formatter {
	...
	public class Mapper {
		...
		protected void populate(Map<String, Object> context, Map<String, String[]> model) {
		 ...
		 * @param values
		 */
		private void populate(Map<String, Object> context, String key, String[] values) {
			if (null != values && values.length > 1) {
				...
				for (String val : values) {
					...
					logger.debug("Added String[] : " + key + count + " - " + val);
					count++;
				}
			} else {
				if ( null != values )
					...
					logger.warn("Null values object passed!");
		 ...
		 */
		protected void update(Map<String, Object> context, String property, Object value) {
	...
	protected static final Logger logger = LoggerFactory.getLogger(Formatter.class);
	...
	public static String getShortSQLDate(String value) {
		...
		// SimpleDateFormat userReadableFormat = new SimpleDateFormat("EEEE,
		// MMMM dd yyyy");
		String apptDateStr = "";
		try {
			// catch crap dates
	...
	public static String UppercaseFirstLetters(String str) {
		...
		if (wordStr != null) {
			// 1. Tv is showing up in the listings too, this needs to be all
			// upper case
		...
		// 2. The letter after an ampersand should be capitalized. An example of
		// this is A&E.
		...
		// 3. A combination of CE with spaces before and after should always be
		// capitalized.
		wordStr = wordStr.replaceAll("(?i) ce ", " CE ");
		// System.out.println("3: " + serviceProviderName);

		// 4. The word "and" with spaces before and after should always appear
		// as lower case.
		wordStr = wordStr.replaceAll("(?i) AND ", " and ");
		// System.out.println("4: " + serviceProviderName);

		// 5. GE as a stand alone set of two letters should always appear as
		// upper case (e.g. GE Consumer Service)
		...
		// System.out.println("PJM: get fixed SP name from SB is " +
		// serviceProviderName);
		}
		return wordStr;
	...
	/**
	 * @TODO UNTESTED UOC as of May 10 2012 populate a context from a list of
	 *       objects, possibly duplicate types - may have multiple customers say
	 * 
	 * @param context
	 * @param model
	 */
	public static Map<String, Object> NestedMapFromObject(Object model) {
		...
		if (null != model) {
			...
			if (model instanceof Map) {
				...
				logger.debug(
						"Cant put string in context : please place in map under a key and pass it instead : " + model);
			} else {
						...
						if (pType.length == 0) {
							try {
								...
								if (ret instanceof Map) {
									...
									mapper.populate(context, key, (String[]) ret);
								} else {
									mapper.update(context, name + propName + count, ret);
								...
								// TODO Auto-generated catch block
								logger.debug("method invoke problem : " + e.toString());
	...
	/**
	 * populate a context from a simple objects/bean using getters : doesnt deal
	 * with collections. null properties not returned<br>
	 * Returns the class and superclass properties<br>
	 * Property names in map are lowercased - eg a property called getMyProp
	 * will be in map as "myProp"
	 * <p>
	 * 
	 * 
	 * @param context
